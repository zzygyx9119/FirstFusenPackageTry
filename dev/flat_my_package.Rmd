---
title: "flat_my_package.Rmd for working package"
...
```{r}
# Initialize Git repository (only run this once)
usethis::use_git()
# After running, commit the initial files manually if prompted
```
...
fusen::inflate(flat_file = "dev/flat_my_package.Rmd", vignette_name = "Get started")
```
...
fusen::inflate(
  pkg = getwd(),
  flat_file = "dev/flat_my_package.Rmd",
  vignette_name = "Get Started with My Package",
  check = TRUE
)
```
